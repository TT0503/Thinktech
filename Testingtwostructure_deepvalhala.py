...
test=pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/Transfer_learning_5_floor/Acceleration/Haver5_In2.xlsx')
#
...
def make_model(input_shape):
    model = tf.keras.Sequential([
        # Input layer (explicitly added)
        tf.keras.layers.Input(shape=input_shape),

        # Flatten layer (reshaping if needed)
        tf.keras.layers.Flatten(),  # Note: No need to specify input_shape here

        # Hidden layers
        tf.keras.layers.Dense(num_hidden_1, activation='sigmoid'),
        tf.keras.layers.Dense(num_hidden_2, activation='sigmoid'),
        tf.keras.layers.Dense(num_hidden_2, activation='sigmoid'),
