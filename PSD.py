...
df = pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/Numerical_Work_NI&code /Data/RFCNN.xlsx')
#
df.columns
result = df.head()
# print (result)
...
while i<631601 and j<631601 :
 ...
 # noise = np.sqrt(noise_power) * np.random.randn(*A.shape)
 # print(noise)
...
directory = '/Users/ahmedatia/ATIA/S D/SHM/Numerical_Work_NI&code /Data/'
...
# Plot the PSD with enhanced visuals
plt.figure()
plt.semilogy(freq[:n//2], psd[:n//2], color=(0, 0.447, 0.741, 0.75), linewidth=1.5)  # Adjust alpha value to 0.75
plt.scatter(freq[:n//2], psd[:n//2], color='black', s=10, label='PSD')
