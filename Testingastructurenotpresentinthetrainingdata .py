...
df = pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/PhD-Thesis-work/All_numerical&code/Numerical_Work/7_Sorey_input_data_neural_networks /RFCNN.xlsx')
test=pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/Transfer_learning_5_floor/Acceleration/Haver5_In4.xlsx')
#
loaded_data = np.load('classificationofsignal-Level_2.npy', allow_pickle=True).item()
x2= x
y2=y
df.columns
result = df.head()
...
#Combining two arrays of two structures togther
# Pad x with zeros only along dimension 2
x= x[:, :, :900]
...
X_train, X_val, y_train, y_val = model_selection.train_test_split (X_combined,Y_combined, test_size=.2 )
y_val_2 = y_val
...
# X_predicted = X_predicted.reshape(6, 1, 900)
...
# # Assuming label_map is defined as mentioned in your question
#
# y_pred_2 = y_pred_prob_2.argmax(axis=1)
# label_map = {'IO': 0, 'LS': 1, 'S': 2, 'CP': 3, 'IO\ufeff': 0}
#
# # Assuming y_pred_2 is a NumPy array or tensor containing multiple predicted class indices
# # Convert each predicted class index to a list of labels using the label_map
# y_pred_labels = [key for pred_index in y_pred_2 for key, value in label_map.items() if value == pred_index]
#
...
# Assuming true_labels is the ground truth labels for the new dataset
