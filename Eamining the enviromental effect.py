...
df = pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/Change_Tempreture/Accelration/merged_file_with_filename.xlsx')
Damage_condition=pd.read_excel('/Users/ahmedatia/ATIA/S D/SHM/Change_Tempreture/Response_Acceleration_Catagories/Response Acceleration Catagories_withoutothercolumns.xlsx')
...
print(result)
# print (result)
Damage_condition = Damage_condition.loc[:,"Damage_condition"]
Joint = df.loc[:,"Joint"]
#
...
df2= df.drop(['Earthquake','Joint','IDL','YG'], axis=1)
earthequake= df.drop(['Joint','U1','IDL','YG'], axis=1)
result = df2.head()
# print (result)
# print(df2.shape)
...
j=900
A2=[]
while i<108106 and j<108106 :
 A= tf.strided_slice(df2, [i], [j])
 A = transpose(A)
 i = i+900
 j= j+900
 A2.append(A)
#coonverting test_Acceleration to list
i=0
j=900
A_test_Acceleration=[]
while  j<7209 :
 A= tf.strided_slice(test_Acceleration, [i], [j])
 A = transpose(A)
 i = i+900
 j= j+900
 A_test_Acceleration.append(A)

# list1 = []
# for i in range(1, 516273):
#     if (i % 901 == 0):
#         list1.append(Damage_condition[i])
list2 =[]
for i in range (1, 108106):
    if(i%900==0) :
...
x = A2
# print (x)
# X_predicted= np.array(A_test_Acceleration, dtype=object)
# x= np.array(x)
# Damage_condition = Damage_condition[:-7]
#
y = np.array(Damage_condition)
#
# np.save('classificationofsignal-Level_2.npy', {'x': x, 'y': y})
...
unique_categories, counts = np.unique(y_val_2, return_counts=True)
...
#
#     # Shallow Dense layers with ReLU activation and dropout
...
#
#     # Add kernel_regularizer to dense layers
#     dense1 = keras.layers.Dense(128, activation="relu", kernel_regularizer=l2(0.01))(flat)
#     dense1 = keras.layers.BatchNormalization()(dense1)
#     dense1 = keras.layers.Dropout(0.5)(dense1)
#
#     dense2 = keras.layers.Dense(64, activation="relu", kernel_regularizer=l2(0.01))(dense1)
#     dense2 = keras.layers.BatchNormalization()(dense2)
...
# # Check if the two arrays are equal element-wise
# # first attempts
# misclassified_sequences_joint_sequence = []
# misclassified_sequences_joint = []
#
# #
# for i in range(len(misclassified_signals_itself)):
#     for j in range(len(A2)):
# # while i in range(len(misclassified_signals_itself)) and j in range(len(A2)):
#         if np.array_equal(misclassified_signals_itself[i], A2[j]):
#             misclassified_sequences_joint.append(Joints[j])
#             misclassified_sequences_joint_sequence.append([j])
...
#
# misclassified_sequences_joint = []
# while i < len(misclassified_signals_itself) :
#     if np.array_equal(misclassified_signals_itself[i], A2[j]):
#         misclassified_sequences_joint.append(Joints[j])
#     i += 1
#     j += 1
#     if i == len(misclassified_signals_itself):
#
#          break
#     while (i+H) < len(A2) or k < len(misclassified_signals_itself) :
#         if np.array_equal(misclassified_signals_itself[k], A2[i+H]):
#             misclassified_sequences_joint.append(Joints[i+H])
#     k += 1
#     H += 1
#
# print(misclassified_sequences_joint)
